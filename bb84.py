...
def measure_qubit_using_basis(qubit: Qubit, basis: bool) -> bool:
    if basis and complex(qubit.state[0, 0]) == 0.7071067811865475+0j or complex(qubit.state[0, 0]) == 0.4999999999999999+0j:
        judge = True
    elif not basis and complex(qubit.state[0, 0]) == 1+0j or complex(qubit.state[0, 0]) == 0j:
        judge = True
    else:
        judge = False 
    qubit.reset()
